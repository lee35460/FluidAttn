...
**대용량 diff(≥100KB) 전달**: 통합 diff 를 tool 인자로 verbatim 전달하지 말고 `reports/review/<step>[-r{N}].diff`(통합) + `reports/review/<step>[-r{N}]-diff/<path 슬래시를 __로 치환>.diff`(파일별 슬라이스)로 먼저 파일에 써서 리뷰어에게는 통합 diff 경로를, skeptic 에게는 자기 파일의 슬라이스 경로만 포인터로 넘긴다(step 12·17·23 에서 쓴 방식 — 세션 tool 출력 한도를 넘기지 않으면서 skeptic 당 전체 diff 재독을 막는다).

**재리뷰 규모**: 수정 커밋을 다시 리뷰할 때, diff 가 ≥100KB 이거나 CRITICAL 경로(git 쓰기·시크릿 마스킹·상태 전이·승격 게이트)를 건드리면 이 커맨드(`/review-code`)로 전체를 다시 돌린다. 그 미만이면 경량 `/review` 로 충분하다(ADR-049). 재리뷰는 "수정이 새로 만든 결함"을 보는 것이 목적이므로 1회차 확정 발견의 해소 여부 확인 + 미검증 상한 초과분 판정을 함께 포함한다.
